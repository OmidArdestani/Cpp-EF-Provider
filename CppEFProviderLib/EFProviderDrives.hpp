...
namespace EFProvider
{
	...
	protected:
		...
		{
			std::list<std::string> propertiVal;
			std::list<std::string> propertiColm;

			// Complete query.
			CAbstractDatabaseModel* model = static_cast<CAbstractDatabaseModel*>(item);
			auto all_property = model->GetAllPropertie();

			for (auto item_itr = all_property.cbegin(); item_itr != all_property.cend(); ++item_itr)
			{
				// check if the object is not a relationship object.
				auto contains_star = item_itr->first.find('*');
				// check if propertie is not id. Becouse the id always is a auto incremental..
				if ((contains_star == std::string::npos) && (this->ToLower(item_itr->first) != "id"))
				{
					propertiColm.push_back(item_itr->first);
					propertiVal.push_back("'" + item_itr->first + "'");
				}
			}

			const std::string command = "insert into " + this->GetTableName() + " (" + this->JoinString(propertiColm, ",") + ") values (" + this->JoinString(propertiVal, ", ") + ");";
